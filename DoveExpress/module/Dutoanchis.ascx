<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Dutoanchis.ascx.cs" Inherits="module_Dutoanchis" %>
<%@ Register TagPrefix="custom" Namespace="ITCLIB.Filter" %>
<telerik:RadScriptBlock ID="RadScriptBlockChi" runat="server">   
<script type="text/javascript">
    function RowDblClickChi(sender, eventArgs) {
            var CanEdit = "<%=ITCLIB.Security.Security.CanEditModule("DUTOANSXKDs") %>";
            if ((eventArgs.get_tableView().get_name() == "MasterTableViewChi") && (CanEdit == "True")) {
                sender.get_masterTableView().editItem(eventArgs.get_itemIndexHierarchical());
            }
    }
    </script>
<script type="text/javascript">
    function PopUpShowingChi(sender, eventArgs) {
...
<telerik:RadAjaxLoadingPanel Skin="Vista" ID="RadAjaxLoadingPanelCHI" runat="server" />
<telerik:RadGrid ID="RadGridCHI" runat="server" Skin="Windows7" 
    AllowPaging="True" PageSize="20" AllowSorting="True" 
    AllowFilteringByColumn="True" GridLines="None" ShowStatusBar="true"
    AutoGenerateColumns="False"  
    AllowMultiRowEdit="True" AllowAutomaticDeletes="True" 
    AllowAutomaticInserts="True" AllowAutomaticUpdates="True" 
    DataSourceID="CHIsDataSource" ShowFooter="True"
    ondatabound="RadGridCHI_DataBound" 
    onitemdeleted="RadGridCHI_ItemDeleted" oniteminserted="RadGridCHI_ItemInserted" 
    onitemupdated="RadGridCHI_ItemUpdated" 
    onitemcommand="RadGridCHI_ItemCommand" 
    onitemdatabound="RadGridCHI_ItemDataBound">
    ...
    <MasterTableView Name="MasterTableViewChi" CommandItemDisplay="Top" DataSourceID="CHIsDataSource" DataKeyNames="PK_ID" ClientDataKeyNames="PK_ID" EditMode="InPlace" NoMasterRecordsText="Không có dữ liệu">
        <CommandItemTemplate>
                    <div style="padding: 5px 5px;float:left;width:auto">
                        <b><asp:Label ID="lblTitle" ForeColor ="Red" runat="server"></asp:Label></b>&nbsp;&nbsp;&nbsp;&nbsp;       
                        <asp:LinkButton ID="LinkButton2" runat="server" CommandName="InitInsert" Visible='<%# !RadGridCHI.MasterTableView.IsItemInserted %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/AddRecord.gif" />Thêm</asp:LinkButton>&nbsp;&nbsp;                 
        ...
        <Columns>
                ...
                <telerik:GridButtonColumn HeaderStyle-Width ="40px" ConfirmText="Bạn muốn xóa dự toán chi?" ConfirmDialogType="RadWindow"
        ...
        <ClientSettings>
            ...
            <ClientEvents OnPopUpShowing="PopUpShowingChi" />
            <ClientEvents OnRowDblClick="RowDblClickChi"/>
            <ClientEvents OnGridCreated="RadGridCHICreated" />
        </ClientSettings>
        <SortingSettings SortedAscToolTip="Sắp xếp tăng dần" 
            SortedDescToolTip="Sắp xếp giảm dần" SortToolTip="Click để sắp xếp" />
        <StatusBarSettings LoadingText="Đang tải..." ReadyText="Sẵn sàng" />
</telerik:RadGrid>
<asp:SqlDataSource ID="CHIsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:CNCREPORTConnectionString %>"
        DeleteCommand="DELETE FROM DUTOANTHUCHI WHERE PK_ID = @PK_ID" 
        InsertCommand="INSERT INTO DUTOANTHUCHI (FK_NGUONKINHPHI,TONGTIEN,MOTA,FK_DONVI,NAM,LOAI) VALUES (@FK_NGUONKINHPHI,@TONGTIEN,@MOTA, @FK_DONVI,@NAM,2)"
        SelectCommand="SELECT DUTOANTHUCHI.PK_ID,DUTOANTHUCHI.FK_NGUONKINHPHI,DUTOANTHUCHI.TONGTIEN,DUTOANTHUCHI.MOTA,DUTOANTHUCHI.FK_DONVI,DUTOANTHUCHI.NAM,DMNGUONKINHPHI.C_NAME as NGUONKINHPHI FROM DUTOANTHUCHI LEFT OUTER JOIN DMNGUONKINHPHI ON DMNGUONKINHPHI.PK_ID = DUTOANTHUCHI.FK_NGUONKINHPHI WHERE NAM =@NAM AND LOAI=2 AND FK_DONVI=@FK_DONVI"
