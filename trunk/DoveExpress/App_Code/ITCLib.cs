...
namespace ITCLIB
{
    ...
    namespace SecurityPrivate
    {
        public class security
        {
            public struct StructPermission
            {
                public int ID;
                public int Permission;               
            }
            public static string Get_Rf(string id, string field_id, string field_show, string table)
            {
                string SelectSQL = "SELECT " + field_show + " FROM " + table + " where " + field_id + "=" + id;
                DataTable oDataTable = new DataTable();
                ITCLIB.Admin.SQL SelectQuery = new ITCLIB.Admin.SQL();
                oDataTable = SelectQuery.query_data(SelectSQL);
                if (oDataTable.Rows.Count != 0)
                {
                    return oDataTable.Rows[0][field_show].ToString();
                }
                else
                {
                    return "";
            ...
            public static string[] Rf2Ar_One(string rf)
            {
                if (rf.EndsWith(";"))
                {
                ...
                {
                    string[] temp = rf.Split(';');
                    return temp;
                }
            }
            public static StructPermission[] Rf2Ar_Two(string rf)
            {
                if (rf.EndsWith(";"))
                {
                    rf = rf.Substring(0, rf.Length - 1);
                ...
                if (rf == "0:0" || rf=="")
                {
                    StructPermission temp1;
                    temp1.ID =0;
                    temp1.Permission = 0;
                    StructPermission[] temp = new StructPermission[1];
                    temp[0] = temp1;
                    return temp;
                }
                else
                {
                    string[] temp1 = rf.Split(';');
                    StructPermission[] temp = new StructPermission[temp1.Length];
                    StructPermission temp2;
                    for (int i = 0; i < temp1.Length; i++)
                    {
                        try
                        {
                            string[] str = temp1[i].Split(':');
                            temp2.ID = int.Parse(str[0]);
                            temp2.Permission = int.Parse(str[1]);
                            temp[i] = temp2;
                        }
                        catch
                        { 
            ...
            public static bool CheckPermission_One(string rf, int ID)
            {
                string[] check = Rf2Ar_One(rf);
                ...
                else
                {
                    ...
                    {
                        if (int.Parse(check[i]) == ID) temp = true; 
                    }
                    return temp;
                }
            }
            public static bool CheckPermission_Two(string rf, int ID,int op) //op: 0-All,1-View,2-Edit,3-Del,4-Other
            {
                StructPermission[] check = Rf2Ar_Two(rf);
                if (check.Length == 1 && check[0].ID == 0 && check[0].Permission == 0)
                {
                    return true;
                }
                else
                {
                    ...
                    for (int i = 0; i < check.Length; i++)
                    {
                        if (check[i].ID == ID && check[i].Permission == op)
                        { 
                            temp = true; 
                        }
                    }
    ...
    namespace Admin
    {
        ...
        public class Email
        {
            public static string Send(string sEmail)
            {
                ...
                {
                    string SelectCompany = "Select T_COMPANY.* FROM T_COMPANY WHERE T_COMPANY.PK_ID= " + HttpContext.Current.Session["CompanyID"];
                    DataTable oDataTableCompany = new DataTable();
                    ITCLIB.Admin.SQL SelectQueryCompany = new ITCLIB.Admin.SQL();
                    oDataTableCompany = SelectQueryCompany.query_data(SelectCompany);
                    if (oDataTableCompany.Rows.Count != 0)
                    {
                        int port = int.Parse(oDataTableCompany.Rows[0]["Email_Port"].ToString());
                        string host = oDataTableCompany.Rows[0]["Email_Smtp"].ToString();
                        string password = oDataTableCompany.Rows[0]["Email_Password"].ToString();
                        string username = oDataTableCompany.Rows[0]["Email_User"].ToString();
                        string SendFrom = oDataTableCompany.Rows[0]["Email"].ToString();
            ...
            public static string Send_Email(string SendTo, string Subject, string Body, string Sender, bool isHtml)
            {
                System.Text.RegularExpressions.Regex regex = new System.Text.RegularExpressions.Regex(@"\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*");
                ...
                else
                {
                  ...
                  if (oDataTableCompany.Rows.Count != 0)
                  {
                      ...
                      SmtpClient client = new SmtpClient(host, port);
                      client.EnableSsl = true;
                      MailAddress from = new MailAddress(SendFrom, Sender);
    ...
    namespace Security
    {
        ...
        public class Security
        {
            ...
            public static void CheckPermissionReport(int guID)
            {
                string SelectPK_REPORT = "Select REPORT.PK_ID FROM REPORT";
                DataTable oDataTable = new DataTable();
                ITCLIB.Admin.SQL SelectQuery = new ITCLIB.Admin.SQL();
                oDataTable = SelectQuery.query_data(SelectPK_REPORT);
                string InsertT_USERTYPE_REPORT = "";
                foreach (DataRow oRow in oDataTable.Rows)
                {
                    string SelectC_LEVELPERMISSION = "Select C_LEVELPERMISSION FROM GROUPUSER_REPORT WHERE FK_GROUPUSER=" + guID + " AND FK_REPORT=" + oRow["PK_ID"];
                    DataTable oDataTable1 = new DataTable();
                    ITCLIB.Admin.SQL SelectQuery1 = new ITCLIB.Admin.SQL();
                    oDataTable1 = SelectQuery1.query_data(SelectC_LEVELPERMISSION);
                    if (oDataTable1.Rows.Count == 0)
                    {
                        InsertT_USERTYPE_REPORT += ";INSERT INTO GROUPUSER_REPORT(FK_GROUPUSER,FK_REPORT,C_LEVELPERMISSION) VALUES(" + guID + "," + oRow["PK_ID"] + ",0)";
                    }
                }
                if (InsertT_USERTYPE_REPORT != "")
                {
                    ITCLIB.Admin.SQL result = new ITCLIB.Admin.SQL();
                    result.ExecuteNonQuery(InsertT_USERTYPE_REPORT);
                }
            }
            ...
            {
                if (HttpContext.Current.Request.QueryString["config"] == "configCNC")
                {
            ...
            public static bool CanViewReport(string ReportType)
            {
                bool Result = false;
                int permission = 0;
                int guID;
                ...
                if (session["User"] != null)
                {
                    ...
                    {
                        string SelectC_LEVELPERMISSION = "Select GROUPUSER_REPORT.C_LEVELPERMISSION FROM GROUPUSER_REPORT INNER JOIN REPORT ON GROUPUSER_REPORT.FK_REPORT = REPORT.PK_ID WHERE GROUPUSER_REPORT.FK_GROUPUSER=" + guID + "AND REPORT.C_TYPE='" + ReportType + "'";
                        DataTable oDataTable = new DataTable();
                        ITCLIB.Admin.SQL SelectQuery1 = new ITCLIB.Admin.SQL();
                        oDataTable = SelectQuery1.query_data(SelectC_LEVELPERMISSION);
                        if (oDataTable.Rows.Count != 0)
            ...
            public static bool CanAddReport(string ReportType)
            {
                int permission = 0;
                int guID;
                System.Web.SessionState.HttpSessionState session = System.Web.HttpContext.Current.Session;
            ...
            public static bool CanEditReport(string ReportType)
            {
                int permission = 0;
                int guID;
                System.Web.SessionState.HttpSessionState session = System.Web.HttpContext.Current.Session;
            ...
            public static bool CanDeleteReport(string ReportType)
            {
                int permission = 0;
                int guID;
                System.Web.SessionState.HttpSessionState session = System.Web.HttpContext.Current.Session;
            ...
            public static bool CanPrintReport(string ReportType)
            {
                int permission = 0;
                int guID;
                System.Web.SessionState.HttpSessionState session = System.Web.HttpContext.Current.Session;
