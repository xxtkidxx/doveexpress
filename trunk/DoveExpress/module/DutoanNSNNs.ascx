<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DutoanNSNNs.ascx.cs" Inherits="module_DutoanNSNNs" %>
<%@ Register TagPrefix="custom" Namespace="ITCLIB.Filter" %>
<telerik:RadScriptBlock ID="RadScriptBlockNSNN" runat="server">   
<script type="text/javascript">
    function RowDblClickNSNN(sender, eventArgs) {
            var CanEdit = "<%=ITCLIB.Security.Security.CanEditModule("DUTOANSXKDs") %>";
            if ((eventArgs.get_tableView().get_name() == "MasterTableViewNSNN") && (CanEdit == "True")) {
                sender.get_masterTableView().editItem(eventArgs.get_itemIndexHierarchical());
            }
    }
    </script>
<script type="text/javascript">
    function PopUpShowingNSNN(sender, eventArgs) {
...
<telerik:RadAjaxLoadingPanel Skin="Vista" ID="RadAjaxLoadingPanelNSNN" runat="server" />
<telerik:RadGrid ID="RadGridNSNN" runat="server" Skin="Windows7" 
    AllowPaging="True" PageSize="20" AllowSorting="True" 
    AllowFilteringByColumn="True" GridLines="None" ShowStatusBar="true"
    AutoGenerateColumns="False"  
    AllowMultiRowEdit="True" AllowAutomaticDeletes="True" 
    AllowAutomaticInserts="True" AllowAutomaticUpdates="True" 
    DataSourceID="NSNNsDataSource" ShowFooter="True"
    ondatabound="RadGridNSNN_DataBound" 
    onitemdeleted="RadGridNSNN_ItemDeleted" oniteminserted="RadGridNSNN_ItemInserted" 
    onitemupdated="RadGridNSNN_ItemUpdated" 
    onitemcommand="RadGridNSNN_ItemCommand" 
    onitemdatabound="RadGridNSNN_ItemDataBound">
    ...
    <MasterTableView Name="MasterTableViewNSNN" CommandItemDisplay="Top" DataSourceID="NSNNsDataSource" DataKeyNames="PK_ID" ClientDataKeyNames="PK_ID" EditMode="InPlace" NoMasterRecordsText="Không có dữ liệu">
        <CommandItemTemplate>
                    <div style="padding: 5px 5px;float:left;width:auto">
                        <b><asp:Label ID="lblTitle" ForeColor ="Red" runat="server"></asp:Label></b>&nbsp;&nbsp;&nbsp;&nbsp;       
                        <asp:LinkButton ID="LinkButton2" runat="server" CommandName="InitInsert" Visible='<%# !RadGridNSNN.MasterTableView.IsItemInserted %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/AddRecord.gif" />Thêm</asp:LinkButton>&nbsp;&nbsp;                 
        ...
        <Columns>
                ...
                <telerik:GridButtonColumn HeaderStyle-Width ="40px" ConfirmText="Bạn muốn xóa dự toán nộp ngân sách nhà nước?" ConfirmDialogType="RadWindow"
        ...
        <ClientSettings>
            ...
            <ClientEvents OnPopUpShowing="PopUpShowingNSNN" />
            <ClientEvents OnRowDblClick="RowDblClickNSNN"/>
            <ClientEvents OnGridCreated="RadGridNSNNCreated" />
        </ClientSettings>
        <SortingSettings SortedAscToolTip="Sắp xếp tăng dần" 
            SortedDescToolTip="Sắp xếp giảm dần" SortToolTip="Click để sắp xếp" />
        <StatusBarSettings LoadingText="Đang tải..." ReadyText="Sẵn sàng" />
</telerik:RadGrid>
<asp:SqlDataSource ID="NSNNsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:CNCREPORTConnectionString %>"
        DeleteCommand="DELETE FROM DUTOANTHUCHI WHERE PK_ID = @PK_ID" 
        InsertCommand="INSERT INTO DUTOANTHUCHI (FK_NGUONKINHPHI,TONGTIEN,MOTA,FK_DONVI,NAM,LOAI) VALUES (@FK_NGUONKINHPHI,@TONGTIEN,@MOTA, @FK_DONVI,@NAM,3)"
        SelectCommand="SELECT DUTOANTHUCHI.PK_ID,DUTOANTHUCHI.FK_NGUONKINHPHI,DUTOANTHUCHI.TONGTIEN,DUTOANTHUCHI.MOTA,DUTOANTHUCHI.FK_DONVI,DUTOANTHUCHI.NAM,DMNGUONKINHPHI.C_NAME as NGUONKINHPHI FROM DUTOANTHUCHI LEFT OUTER JOIN DMNGUONKINHPHI ON DMNGUONKINHPHI.PK_ID = DUTOANTHUCHI.FK_NGUONKINHPHI WHERE NAM =@NAM AND LOAI=3 AND FK_DONVI=@FK_DONVI"
